...
class Harvester(object):
    ...
    def _check_sync(self, core=None, query=DEFAULT_QUERY, fq="_timestamp:[* TO *]"):
        ...
        logging.debug("SOURCE: counts=%s time stamp min=%s max=%s mean=%s" % (counts1, timestamp_min1, timestamp_max1, timestamp_mean1))
        logging.debug("TARGET: counts=%s time stamp min=%s max=%s mean=%s" % (counts2, timestamp_min2, timestamp_max2, timestamp_mean2))
